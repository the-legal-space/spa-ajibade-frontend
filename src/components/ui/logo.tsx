 ...
 */
export function Logo({ firmName, descriptor, className }: { firmName: string; descriptor?: string; className?: string }) {
  return (
    <span className={cn("inline-flex items-center gap-2 text-white", className)}>
      ...
      <span className="flex flex-col leading-none">
        ...
        {descriptor ? <span className="mt-0.5 self-end text-[6.5px] tracking-wide text-white/80">{descriptor}</span> : null}
      </span>
    </span>
  );
}
